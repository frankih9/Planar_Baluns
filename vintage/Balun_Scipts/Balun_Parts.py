...
def SQ(W, tl=37):
    ...
    ###############################     
    SQ = gdspy.Cell('SQ',exclude_from_current=False)
...
def J(W, S, tl = 37, ext = None):
    ...
    ################################# 
    J = gdspy.Cell('J',exclude_from_current=False)
    J.add(shape)
    
    
def TR(L, W, S, Pri, Sec, tl=37):
    '''
    ...
    TR = gdspy.Cell('TR',exclude_from_current=False)
    for ang in range(8):
...
def VIA(m, w, s, vl=36):
    ...
    #########################################################       
    v1 = gdspy.Cell('VIA_1',exclude_from_current=False)
    var = gdspy.Cell('VIA_ARR',exclude_from_current=False)
    ...
    #################################
    var.add(gdspy.CellArray('VIA_1', m, m, (w+s,w+s), (loc,loc))) 
    var.flatten()
...
def X(W, S, tl=37, bl=33, ext = None):
    ...
    #####################################         
    X = gdspy.Cell('X',exclude_from_current=False)
    ...
    ##################################################
    XM = gdspy.Cell('XM',exclude_from_current=False) 
    X_ref = gdspy.CellReference('X', (0,0), x_reflection=True)
